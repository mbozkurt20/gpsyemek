                ...
                <form action="{{ route('admin.user.update', $user) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="row">
                            ...
                            <div class="db-field-down-arrow">
                                <select name="role" class="db-field-control appearance-none @error('role') invalid @enderror">
                                    ...
                                    @foreach($roles as $key => $role)
                                        <option value="{{ $role->id }}" {{ (old('role') == $role->id || $user->getrole->id == $role->id) ? 'selected' : '' }}>{{ $role->name }}</option>
                                    @endforeach
                                    @endif
                                </select>
                            </div>
                        ...
                        <div class="form-col-12 sm:form-col-6 md:form-col-4">
                            ...
                            <input type="text" name="address" id="address" class="db-field-control @error('address') invalid @enderror" value="{{ old('address', $user->address) }}"></input>
