        ...
        <div class="col-12">
            <div class="db-card">
                <div class="db-card-header border-none">
                    <h3 class="db-card-title">{{ __('withdraw.withdraw_details') }}</h3>
                    <div class="db-card-filter">
                        @can('withdraw_create')
                            <a href="{{ route('admin.withdraw.create') }}" class="db-btn h-[38px] text-white bg-primary">
                                <i class="fa-solid fa-circle-plus"></i>
                                <span>{{ __('withdraw.add_withdraw') }}</span>
                            </a>
                        @endcan
                    </div>
                </div>
                ...
                <div class="db-table-responsive">
                    <table class="db-table striped" id="maintable" data-url="{{ route('admin.withdraw.get-withdraw') }}">
                        <thead class="db-table-head">
                            <tr class="db-table-head-tr">
                                <th class="db-table-head-th">{{ __('levels.name') }}</th>
                                <th class="db-table-head-th">{{ __('withdraw.payment_type') }}</th>
